  ...
  int main(int argc, char **argv) {
     ...
     // ROS_INFO_STREAM("urdf: \n" <<red_xmlStr);
      ...
      spawn_model_req.initial_pose.position.x = 1.5; //1.5;   //2 bob, right under robot
      ...
      apply_wrench_req.wrench.force.x = -5.1;  //bob -5.1;   //-4.1;  //bob, stop belt move
      ...
      while (ros::ok()){
          ...
          spawn_model_req.initial_pose.position.y = 0.26; //bob (float)rand()/(float)(RAND_MAX) * 0.4;  // random between -0.4 to 0.4 bob
